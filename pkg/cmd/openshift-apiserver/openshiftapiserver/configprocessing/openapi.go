...
var (
	...
	operationIDAndTags = func(r *restful.Route) (string, []string, error) {
		op := r.Operation
		path := r.Path
...
func DefaultOpenAPIConfig() *openapicommon.Config {
	...
	defaultConfig := genericapiserver.DefaultOpenAPIConfig(openapigenerated.GetOpenAPIDefinitions, defNamer)
	defaultConfig.Info = &specInfo
	defaultConfig.IgnorePrefixes = ignorePrefixes
	defaultConfig.GetOperationIDAndTags = operationIDAndTags
	defaultConfig.SecurityDefinitions = &spec.SecurityDefinitions{
...
func DefaultOpenAPIV3Config() *openapicommon.OpenAPIV3Config {
	defNamer := apiserverendpointsopenapi.NewDefinitionNamer(legacyscheme.Scheme, extensionsapiserver.Scheme, aggregatorscheme.Scheme)
	defaultConfig := genericapiserver.DefaultOpenAPIV3Config(openapigenerated.GetOpenAPIDefinitions, defNamer)
	defaultConfig.Info = &specInfo
	defaultConfig.IgnorePrefixes = ignorePrefixes
	defaultConfig.GetOperationIDAndTags = operationIDAndTags
	defaultConfig.SecuritySchemes = map[string]*spec3.SecurityScheme{
		"BearerToken": {
