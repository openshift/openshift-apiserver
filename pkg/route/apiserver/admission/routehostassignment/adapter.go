...
func AllocateHost(ctx context.Context, route *routeinternal.Route, sarClient routecommon.SubjectAccessReviewCreator, hg hostassignment.HostnameGenerator) field.ErrorList {
	...
	errs := hostassignment.AllocateHost(ctx, &external, sarClient, hg)
	if len(errs) > 0 {
		return errs
	}
...
func ValidateHostUpdate(ctx context.Context, route, oldRoute *routeinternal.Route, sarClient routecommon.SubjectAccessReviewCreator) field.ErrorList {
	...
	return hostassignment.ValidateHostUpdate(ctx, &external, &oldExternal, sarClient)
}
