...
func autoConvert_v1_ImageBlobReferences_To_image_ImageBlobReferences(in *v1.ImageBlobReferences, out *image.ImageBlobReferences, s conversion.Scope) error {
	...
	// WARNING: in.Manifests requires manual conversion: does not exist in peer-type
	return nil
}
...
func autoConvert_v1_ImageStreamLayers_To_image_ImageStreamLayers(in *v1.ImageStreamLayers, out *image.ImageStreamLayers, s conversion.Scope) error {
	...
	if in.Images != nil {
		in, out := &in.Images, &out.Images
		*out = make(map[string]image.ImageBlobReferences, len(*in))
		for key, val := range *in {
			newVal := new(image.ImageBlobReferences)
			if err := Convert_v1_ImageBlobReferences_To_image_ImageBlobReferences(&val, newVal, s); err != nil {
				return err
			}
			(*out)[key] = *newVal
		}
...
func autoConvert_image_ImageStreamLayers_To_v1_ImageStreamLayers(in *image.ImageStreamLayers, out *v1.ImageStreamLayers, s conversion.Scope) error {
	...
	if in.Images != nil {
		in, out := &in.Images, &out.Images
		*out = make(map[string]v1.ImageBlobReferences, len(*in))
		for key, val := range *in {
			newVal := new(v1.ImageBlobReferences)
			if err := Convert_image_ImageBlobReferences_To_v1_ImageBlobReferences(&val, newVal, s); err != nil {
				return err
			}
			(*out)[key] = *newVal
		}
