	...
	informer := cache.NewSharedIndexInformer(&cache.ListWatch{
		...
		WatchFunc: func(options metav1.ListOptions) (watch.Interface, error) {
			...
			return watch.Filter(w, func(in watch.Event) (out watch.Event, keep bool) {
				// Transform bookmark events to ImageLayers while preserving annotations.
				// Bookmarks are synthetic events generated by the apiserver's cacher (not stored in etcd)
				// and are never stored in the client-side informer cache. The reflector only uses them
				// to signal sync completion via the InitialEventsAnnotationKey annotation:
				//
				//   case watch.Bookmark:
				//       if meta.GetAnnotations()[metav1.InitialEventsAnnotationKey] == "true" {
				//           watchListBookmarkReceived = true
				//       }
				//       // No store.Add/Update/Delete - event is discarded after checking
				//
				// See https://github.com/kubernetes/kubernetes/blob/8020ce8511aab076f939b104c973b84cb7a1afc3/staging/src/k8s.io/client-go/tools/cache/reflector.go#L1058-L1070
				//
				// The reflector expects all events to be *ImageLayers type (since that's what we passed to
				// SharedIndexInformer), so we must transform bookmarks but preserve the critical annotation.
				if in.Type == watch.Bookmark {
					if m, err := meta.Accessor(in.Object); err == nil {
						// Transform to ImageLayers with only the essential bookmark metadata
...
type ImageLayers struct {
	...
	// Annotations stores metadata needed for watch bookmarks (WatchList protocol).
	// For regular image objects, this is nil to save memory. For bookmark events,
	// it contains the k8s.io/initial-events-end annotation needed by the reflector.
...
func (l *ImageLayers) GetAnnotations() map[string]string               { return l.Annotations }
func (l *ImageLayers) SetAnnotations(annotations map[string]string)    { l.Annotations = annotations }
