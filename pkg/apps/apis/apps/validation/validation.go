...
func ValidateDeploymentConfigSpec(spec appsapi.DeploymentConfigSpec) field.ErrorList {
	...
	if spec.Template == nil {
		...
		allErrs = append(allErrs, validation.ValidatePodTemplateSpecForRC(spec.Template, nil, spec.Selector, spec.Replicas, specPath.Child("template"), validation.PodValidationOptions{})...)
	}
	if spec.Replicas < 0 {
