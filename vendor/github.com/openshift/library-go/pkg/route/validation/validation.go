...
func warnPathWithRewriteTargetAnnotation(route *routev1.Route) []string {
	...
	if _, ok := route.Annotations[rewriteTargetAnnotation]; ok {
		if containsRegexMetaChars(route.Spec.Path) {
			warnings = append(warnings, fmt.Sprintf("spec.path contains regex meta characters, %s may provide an unexpected result", rewriteTargetAnnotation))
		}
		if strings.Contains(route.Spec.Path, `'`) {
			warnings = append(warnings, fmt.Sprintf("spec.path contains a ' %s may produce an unexpected result", rewriteTargetAnnotation))
		}
		if strings.Contains(route.Spec.Path, `"`) {
			warnings = append(warnings, fmt.Sprintf("spec.path contains a \" %s may produce an unexpected result", rewriteTargetAnnotation))
		}
